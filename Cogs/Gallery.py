...
def make_gallery_embeds(images, page, per_page, author=None):
	...
	for img in page_images:  # Only one image per page
		...
		is_staff = img.get('isStaff', False) or author_name.lower() in [
			a.lower() for a in STAFF_AUTHORS
		]
	...
	async def on_submit(self, interaction: discord.Interaction):
		try:
			...
			if 1 <= page <= self.view.max_page:
				...
				embeds = make_gallery_embeds(
					self.view.images, self.view.page, self.view.per_page, self.view.author
				)
				await interaction.response.edit_message(embeds=embeds, view=self.view)
			else:
				await interaction.response.send_message(
					f'Invalid page number. Please enter a number between 1 and {self.view.max_page}.',
					ephemeral=True,
				)
		except ValueError:
...
class GalleryView(discord.ui.View):
	...
	async def reload(self, interaction: discord.Interaction, button: discord.ui.Button):
		...
		if cog:
			...
			if self.author:
				images = [
					img for img in images if self.author.lower() in img.get('author', '').lower()
				]
			self.images = images
...
class Gallery(commands.Cog):
	...
	async def gallery(self, ctx: commands.Context, page_or_author: str = None, author: str = None):
		...
		Logger.info(
			'[Gallery] command used by %s (ID: %s) with args: page_or_author=%s, author=%s',
			ctx.author,
			ctx.author.id,
			page_or_author,
			author,
