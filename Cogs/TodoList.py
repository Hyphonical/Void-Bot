...
TODO_DATA_FILE = 'todo_list.json'
TODO_PERSISTENT_VIEWS_FILE = 'todo_persistent_views.json'


# === Helper function for embed footer ===
def set_embed_footer(embed: discord.Embed, bot: discord.ClientUser = None) -> None:
	"""Set footer on embed with bot name and color"""
...
def save_todo_data(data: Dict[str, Any]) -> None:
	...
	dir_path = os.path.dirname(TODO_DATA_FILE)
	if dir_path:
...
def is_mod_or_admin(user) -> bool:
	...
	if not hasattr(user, 'roles'):
		return False
	# Check if user has any admin role or the moderator role
	return any(role.id in ADMIN_ROLE_IDS or role.id == MODERATOR_ROLE_ID for role in user.roles)
...
class TodoPageNavigationView(discord.ui.View):
	...
	@discord.ui.button(
		label='◀ Previous', style=discord.ButtonStyle.secondary, custom_id='todo_nav_previous'
	)
	...
	@discord.ui.button(
		label='📄 Page', style=discord.ButtonStyle.primary, custom_id='todo_nav_info', disabled=True
	)
	...
	@discord.ui.button(
		label='Next ▶', style=discord.ButtonStyle.secondary, custom_id='todo_nav_next'
	)
	...
	@discord.ui.button(
		...
		custom_id='todo_nav_manage',
		emoji='⚙️',
		row=1,
	)
	async def manage_pages_button(
		self, interaction: discord.Interaction, button: discord.ui.Button
	) -> None:
		# Check if user is mod or admin
		if not is_mod_or_admin(interaction.user):
		...
		set_embed_footer(embed, bot=self.bot.user)
	...
	async def refresh_display(
		...
		if 0 <= current_page_idx < len(pages):
					...
					if channel:
						for message_id in channel_data['message_ids']:
							try:
								...
								logger.warning(
									f'Could not delete old to-do message {message_id}: {e}'
								)
				except Exception as e:
			...
			num_embeds = (
				max(1, (total_items + max_per_embed - 1) // max_per_embed) if total_items > 0 else 1
			)
...
class AddPageModal(discord.ui.Modal, title='➕ Add New Page'):
	...
	async def on_submit(self, interaction: discord.Interaction) -> None:
		...
		await interaction.followup.send(
			f'✅ Added new page: **{new_page["title"]}**', ephemeral=True
		)

		logger.info(
			f"Added new todo page '{new_page['title']}' in channel {self.channel_id} by {interaction.user}"
		)
...
class EditPageTitleModal(discord.ui.Modal, title='✏️ Edit Page Title'):
	...
	def __init__(
		self, bot: commands.Bot, channel_id: int, page_index: int, current_title: str
	) -> None:
		super().__init__()
		self.bot = bot
		self.channel_id = channel_id
...
class DeletePageConfirmView(discord.ui.View):
	...
	async def confirm_delete(
		...
		if len(pages) <= 1:
			await interaction.response.send_message(
				'❌ Cannot delete the last page!', ephemeral=True, delete_after=5
			)
			return
		...
		if 0 <= self.page_index < len(pages):
			...
			logger.info(
				f"Deleted page '{deleted_title}' from channel {self.channel_id} by {interaction.user}"
			)
	...
	async def cancel_delete(
		...
		await interaction.response.send_message(
			'❌ Page deletion cancelled.', ephemeral=True, delete_after=5
		)
...
class TodoModal(discord.ui.Modal, title='✏️ Update To-Do List'):
	...
	async def on_submit(self, interaction: discord.Interaction) -> None:
		...
		try:
			...
			if view is None:
				logger.error(
					'Attempted to send preview message with view=None. This should never happen!'
				)
				await interaction.followup.send(
			...
			await interaction.followup.send(
				'❌ Failed to format task with AI. Check logs.', ephemeral=True
			)
...
"""
	...
	async def update_todo_message(
		...
		if not pages:
			...
			logger.warning(
				f'Invalid current_page index {current_page_idx} in channel {channel_id}, resetting to 0'
			)
			current_page_idx = 0
			channel_data['current_page'] = 0
			save_todo_data(data)
				...
				if channel:
					for message_id in channel_data['message_ids']:
						try:
							...
							await old_message.delete()
							logger.info(
								f'Deleted old to-do message {message_id} in channel {channel_id}'
							)
...
class TodoManageView(discord.ui.View):
	...
	async def add_item(self, interaction: discord.Interaction, button: discord.ui.Button) -> None:
		...
		view = PrioritySelectView(
			self.bot, self.channel_id, action='add', management_message=self.management_message
		)
		await interaction.response.send_message(
			embed=embed, view=view, ephemeral=True, delete_after=30
		)
	...
	async def assign_task(
		...
		if 0 <= current_page_idx < len(pages):
			...
			if not items:
				await interaction.response.send_message(
					'❌ No items on current page!', ephemeral=True, delete_after=10
				)
				return
	...
	async def claim_task(self, interaction: discord.Interaction, button: discord.ui.Button) -> None:
		...
		if 0 <= current_page_idx < len(pages):
			...
			view = TodoClaimSelectView(
				self.bot, self.channel_id, items, current_page_idx, interaction.user
			)
			embed = discord.Embed(
	...
	@discord.ui.button(
		label='Unassign Task', style=discord.ButtonStyle.secondary, emoji='❌', row=2
	)
	async def unassign_task(
		...
		if 0 <= current_page_idx < len(pages):
			...
			assigned_items = [
				...
				if item.get('assigned_to_id') is not None
			]
			...
			view = TodoUnassignSelectView(
				self.bot, self.channel_id, assigned_items, current_page_idx
			)
			embed = discord.Embed(
				title='❌ Unassign Task',
				description='Select a task to remove assignment:',
				color=EMBED_COLOR,
			)
			set_embed_footer(embed, bot=self.bot.user)
			await interaction.response.send_message(
...
class TodoAssignSelectView(discord.ui.View):
	...
	def __init__(
		self, bot: commands.Bot, channel_id: int, items: List[Dict], page_idx: int
	) -> None:
		super().__init__(timeout=300)
		self.bot = bot
		self.channel_id = channel_id
	...
	async def select_callback(self, interaction: discord.Interaction) -> None:
		...
		# Show user selector
		view = UserSelectView(
			self.bot, self.channel_id, self.page_idx, item_idx, item, interaction.user
		)
		...
		if item.get('assigned_to_id'):
			embed.add_field(
				name='⚠️ Currently Assigned To',
				value=f'{item.get("assigned_to_name", "Unknown")}',
				inline=False,
			)
			embed.add_field(
				name='Note', value='This will reassign the task to the new user.', inline=False
			)
...
class UserSelectView(discord.ui.View):
	...
	def __init__(
		...
		user_select = discord.ui.UserSelect(
			placeholder='Select a user to assign...', min_values=1, max_values=1
		)
	...
	async def user_selected(self, interaction: discord.Interaction) -> None:
		...
		if item.get('assigned_to_id') == user.id:
			await interaction.followup.send(
				f'❌ Task is already assigned to {user.display_name}!', ephemeral=True
			)
			return
		...
		if old_assignee:
			...
			logger.info(
				f'{self.assigner} assigned task {self.item_idx + 1} to {user} in channel {self.channel_id}'
			)
...
class TodoClaimSelectView(discord.ui.View):
	...
	def __init__(
		...
		select = discord.ui.Select(
			placeholder='Choose a task to claim...', options=options, min_values=1, max_values=1
		)
		select.callback = self.select_callback
		self.add_item(select)
	...
	async def select_callback(self, interaction: discord.Interaction) -> None:
		...
		if old_assignee:
			embed.add_field(
				name='⚠️ Note',
				value=f'This task was reassigned from **{old_assignee}**',
				inline=False,
			)
...
class TodoUnassignSelectView(discord.ui.View):
	...
	def __init__(
		self, bot: commands.Bot, channel_id: int, assigned_items: List[tuple], page_idx: int
	) -> None:
		super().__init__(timeout=300)
		self.bot = bot
		self.channel_id = channel_id
...
class TodoList(commands.Cog):
	...
	def __init__(self, bot: commands.Bot) -> None:
		...
		if os.path.exists(self.persistent_views_file):
			...
			# Only create directory if there is one (not for root-level files)
			dir_path = os.path.dirname(self.persistent_views_file)
			if dir_path:
				os.makedirs(dir_path, exist_ok=True)
			self.persistent_views_data = []
	...
	async def _restore_persistent_views(self) -> None:
		...
		for view_data in self.persistent_views_data:
			...
			try:
				...
				logger.info(
					f'Restored todo navigation view for message {message_id} in channel {channel_id}'
				)
	...
	def _save_persistent_view(self, channel_id: int, message_id: int) -> None:
		...
		for view_data in self.persistent_views_data:
			if (
				view_data.get('channel_id') == channel_id
				and view_data.get('message_id') == message_id
			):
		...
		if len(self.persistent_views_data) < original_count:
			try:
				with open(self.persistent_views_file, 'w', encoding='utf-8') as f:
					json.dump(self.persistent_views_data, f, indent=2)
				logger.info(
					f'Removed persistent view for message {message_id} in channel {channel_id}'
				)
			except Exception as e:
	...
	@app_commands.command(
		name='todo-update',
		description='📋 Update the to-do list for this channel with interactive buttons.',
	)
