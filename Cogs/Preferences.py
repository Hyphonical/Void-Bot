...
class PreferencesSystem(commands.Cog):
    ...
    async def preferences_slash(self, interaction: discord.Interaction):
        ...
        view = PreferencesView(interaction.user.id, interaction.guild, interaction.user)  # Pass user
    ...
    def get_preferences_help_embed(self, ctx_or_interaction, user):
        ...
        embed.add_field(name="Options", value="🔔 Toggle Weekly Summary Notifications – Opt-in to receive role <@&1427011632164376617> for weekly updates.", inline=False)
...
class PreferencesView(discord.ui.View):
    def __init__(self, user_id, guild, user):  # Add user parameter
        ...
        has_role = any(role.id == WEEKLY_SUMMARY_ROLE_ID for role in user.roles)
        label = "Disable Weekly Summary Notifications" if has_role else "Enable Weekly Summary Notifications"
...
class ToggleWeeklySummaryButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if not role:
            await interaction.response.send_message("Weekly summary role not found.", ephemeral=True)
            return
        if role in member.roles:
            ...
            status = "enabled"
        await interaction.response.send_message(f"Weekly summary notifications {status}.", ephemeral=True)
